...
-- Improved: Added favorited detection support based on InventoryController analysis
...
function InventoryWatcher.new()
    ...
    self._favSig    = mkSignal()  -- NEW: (favoritedCounts)
...
-- NEW: Check if entry is favorited
function InventoryWatcher:_isFavorited(entry)
    if not entry then return false end
    -- Based on InventoryController analysis - check multiple possible locations
...
-- NEW: Subscribe to favorited counts changes
...
-- NEW: Get favorited counts by type
...
-- NEW: Get all favorited items of a specific type
...
-- NEW: Check if specific item is favorited by UUID
